...
---
### 5. 메서드 및 데이터 처리 
- **GET/POST 선택**: 맛집 검색(Kakao)은 데이터 조회를 위해 `GET`을, LLM 요청은 보안과 데이터 크기를 고려해 `POST`를 사용했습니다.
- **입력 정규화**: LLM이나 사용자가 입력한 도시명에서 '시', '군' 및 불필요한 공백을 제거하여 검색 API의 정확도를 높였습니다.

---
### 6. 재시도 및 예외 처리 
- **재시도 정책**: 응답 실패 시 최대 3회 재시도하며, 2회차부터는 JSON 형식을 강조하는 보강 프롬프트를 추가합니다.
- **데이터 부재 대응**: 맛집 정보가 0건일 경우 '데이터 없음'을 표기하고 리포트를 정상 출력하여 시스템 안정성을 확보했습니다.
- **캐싱 전략(제안)**: 향후 동일 날짜/도시 요청 시 API 비용 절감을 위해 로컬 파일 기반의 캐싱(TTL 24h) 도입을 설계에 반영할 예정입니다.

---
### 7. 보안 
- **시크릿 관리**: `.env`를 사용하며, 운영 환경에서는 AWS Secrets Manager 사용을 권장합니다.
---
...
API 호출 시 **401 Unauthorized** 또는 **403 Forbidden** 에러가 발생할 경우 다음 사항을 확인하세요.

1. **401 Unauthorized (인증 오류)**
   - `.env` 파일에 API Key가 정확히 입력되었는지 확인하세요.
   - API Key의 앞뒤에 공백이 포함되어 있지 않은지 확인하세요.
   - API Key가 만료되었거나 비활성화되지 않았는지 서비스 콘솔에서 확인하세요.

2. **403 Forbidden (권한 오류)**
   - 해당 API 서비스가 활성화(Enable) 상태인지 확인하세요.
   - API 호출 한도(Quota)를 초과하지 않았는지 확인하세요.
   - 사용 중인 API Key에 해당 리소스에 접근할 수 있는 적절한 권한(Scope)이 부여되었는지 확인하세요.
   - 요청 헤더(Headers)에 필요한 인증 정보가 올바른 형식으로 포함되었는지 확인하세요.
